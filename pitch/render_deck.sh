...
npx --yes @marp-team/marp-cli@latest deck.md --no-stdin --allow-local-files -o deck.html </dev/null
echo "rendered deck.html"
if npx --yes @marp-team/marp-cli@latest deck.md --no-stdin --allow-local-files --pdf -o deck.pdf </dev/null 2>/dev/null; then
